...
void ktTokenizerRun(const char* contents, TokenList* out_list)
{
	...
	while (true)
	{
		...
		if (isblank(curr))
		{
			// Skip all whitespaces, but add one TOKEN_WHITESPACE to the token list.
			while (isblank(curr))
			{
				advance();
			}
			ktTokenListAppend(out_list, ktTokenCreateSymbol(TOKEN_WHITESPACE));
		...
		else if (curr == TOKEN_SUB_SYMBOL)
		{
			...
			bool isNegate = ktTokenListIsEmpty(out_list)
