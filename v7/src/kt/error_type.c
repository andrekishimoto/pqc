...
const char* ktErrorDescription(ktErrorType type)
{
	...
	case KT_ERROR_TOKENIZER_EXTRA_DECIMAL_POINT:
		return "A valid number should have zero or only one decimal point.";
	...
	case KT_ERROR_PARSER_DID_YOU_MEAN_LET:
		return "Did you mean to use the command LET?";

	case KT_ERROR_PARSER_NO_MORE_TOKENS:
		return "There are no more tokens to parse (unexpected EOF).";
	...
	case KT_ERROR_INTERPRETER_EXPR_STMT_OPEN_PAREN:
		return "The expression contains one or more opened parentheses.";
