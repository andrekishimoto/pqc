...
#endif // #if _DEBUG_PARSER_TOKENLIST
...
bool advance()
{
	...
	{
		// If we reached this point, then we found an error!
		g_parser->callback->error(2, "There are no more tokens to parse (unexpected KT_TOKEN_EOF).");
...
bool consume(ktTokenType expected)
{
	...
	else
	{
		// If we reached this point, then we found an error!
		char buffer[128] = { 0 };
		snprintf(buffer, 128, "Incorrect token. Expected: %s, got: %s.", KT_TOKEN_TYPE_STR[expected], KT_TOKEN_TYPE_STR[g_parser->token->type]);
...
void expr()
{
	...
	while (g_parser->token->type == KT_TOKEN_ADD
...
void term()
{
	...
	while (g_parser->token->type == KT_TOKEN_MUL
