	...
	while (true)
	{
		...
		else if (isalpha(curr))
		{
			...
			else // We are inside a string - check for single word and/or valid statements.
			{
				...
				size_t rpnLength = ktMax(strLength, strlen(TOKEN_STMT_RPN_VALUE)); // TODO: *** DEBUG CODE - REMOVE ME FROM FINAL BUILD. ***
				...
				else if (strncmp(str, TOKEN_STMT_RPN_VALUE, rpnLength) == 0) // TODO: *** DEBUG CODE - REMOVE ME FROM FINAL BUILD. ***
				{
